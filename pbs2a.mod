 ...
 * Insead, the solution information is printed into a file that can executed by pyhton
 *********************************************/
...
execute {
  ...
  if (file_export != "") {
	...
	var f = new IloOplOutputFile("out_pbs2a.txt");
	f.writeln("z=",z);
	f.writeln("FlowTime=",FlowTime);
	f.writeln("NumberOfMovements=",NumberOfMovements);
