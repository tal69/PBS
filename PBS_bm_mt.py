...
#              next state
#
# Usage        Command line : python puzzle_block <Lx> <Ly> <k>
...
import math
from PBSCom import *
import create_forbid_table_bm as cft

debug = False # set to true to increase integerity checking while the table is created
...
def puzzle_solve(Lx , Ly ,k, Terminals, name, min_energy=False):
    ...
    # F is a set of 8-tuples
    # If the tuple (o1x, o1y, d1x,d1y, o2x, o2y, d2x,d2y) is a member of F then
    # moving one load from (o1x,o1y) to (d1x,d1y)  and another load from
    # (o2x,o2y) to (d2x,d2y) at the same step is forbiden
    #F = pickle.load( open( "F_"+str(Lx)+"x"+str(Ly)+".p", "rb" ) )
    F = cft.create_table(Lx,Ly)
        ...
        for cs in S[dist]:  # scan all the state in the previous dictionary
            ...
            while True:
                ...
                if curr == k:
                    ...
                    if debug and II in nextE:  # just for debugging
                        print("Panic: the req load was moved to the new location of an an escort")
                        exit(-1)

                    if debug and (II[0] < 0 or II[1] <0):   # just for debugging
                    ...
                    # here we will extend the lengh of  the entry by a decison about one movement and reinsert it to the tree
                    ...
                    for x in range(Lx):
                        ...
                        Cross= False
                        for i in range(curr):
                            if (E[curr][0],E[curr][1],x, E[curr][1], E[i][0], E[i][1], nextE[i][0], nextE[i][1] )  in F:
                                Cross=True
                                break
                    ...
                    for y in range(Ly):
                        if y != E[curr][1]:  # if  there is no movment we already added it as an horizontal movment
    ...
    for i in range(len(S)):
        for k,e in S[i].items():
            if debug and k in C:  # for deubugging only
    ...
    pickle.dump( C, open( "BM_"+name, "wb" ) )
